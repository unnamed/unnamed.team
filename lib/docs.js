 ...
 * and converts it to HTML, and sets it into the
 * "into" argument
 *
 * @param {DocTree} tree The tree to convert to HTML
 * @param {DocTree} into The target tree to write HTML
 * @returns {Promise<DocTree>} The "into" argument, with
 * all the converted HTML data
 */
export async function toHtml(tree, into = {}) {
  for (const [ filename, node ] of Object.entries(tree)) {
    into[filename] = isContent(node)
      ? await markdownToHtml(node)
      : await toHtml(node);
  }
  return into;
}